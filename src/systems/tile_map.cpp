...
void tileMapRender(void*)
{
    ...
    group->each({TILE_SET,TILE_MAP}, [&](Entity* entity){
                ...
                for (int x = 0; x < tileMap->width; x++) {
                    ...
                    if (tileSet->inBounds(tile)) {
                        ...
                        if(tileMap->models.find(tileData.texture) == tileMap->models.end()) {
                            tileMap->models[tileData.texture] = std::make_shared<Model2D>();
                        }
                        std::shared_ptr<Model2D> model = tileMap->models[tileData.texture];
        ...
        for(auto& model : tileMap->models) {
            ...
            tileMap->shader->use();
            tileMap->shader->set("SCREEN_RESOLUTION", Vector2((float)Graphics::getViewportWidth(), (float)Graphics::getViewportHeight()));
