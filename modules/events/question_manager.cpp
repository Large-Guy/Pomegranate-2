...
std::unordered_map<QuestionID, std::vector<QuestionFunction>> Question::_events;
std::unordered_map<std::string, QuestionID> Question::_eventIndex;
QuestionID Question::_eventCounter = 0;

void Question::answer(EventID id, const QuestionFunction& callback) {
    Question::_events[id].push_back(callback);
}

void Question::answer(const std::string &name, const QuestionFunction& callback) {
